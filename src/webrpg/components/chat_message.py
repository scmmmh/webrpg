...
def calculate_dicerolls(target, value, old_value, initiator):
    ...
    if target.session.dice_roller == 'd20':
        return calculate_d20_dice(value)
    elif target.session.dice_roller == 'eote':
        return calculate_eote_dice(value)
    else:
        return value


def calculate_d20_dice(message):
    """Calculate the D20 dice substitutions for the ``message``.

    :params message: The message to process
    :type message: ``unicode``
    :return: The substituted dice rolls
    :rtype: ``unicode``
    """
    substitutions = []
...
def calculate_eote_dice(message):
    """Calculate the Edge-of-the-Empire dice substitutions for the ``message``.
